 ...
 * Implements SpriteGeneratorInterface for consistency
 */
abstract class AbstractSpriteGenerator implements SpriteGeneratorInterface
{
    /** @var EntityType */
     ...
     * Default implementation checks if image_url matches getImageUrl()
    ...
    {
        // Remove orientation suffix for comparison
        $baseImageUrl = $entityType->image_url;
        foreach (array_keys($this->getRotationAngles()) as $suffix) {
            if (substr($baseImageUrl, -strlen($suffix)) === $suffix) {
                $baseImageUrl = substr($baseImageUrl, 0, -strlen($suffix));
                break;
            }
        }

        return $baseImageUrl === $this->getImageUrl();
    }
}
